    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4 py-4">
		...
		<div class="container py-4">
				...
				<div class="form-group">
			    	<div class="container">
			          	<div class="row justify-content-md-end">
			                ...
			                <select id="selectGroupCode" name="t" class="form-control">
				            	<option value="0" <c:if test="${0 eq param.t}">selected</c:if>>전체</option>						
				                <c:forEach var="item" items="${groupCodeList}">
				                <option value="${item.group_cd}" <c:if test="${item.group_cd eq param.t}">selected</c:if>>${item.group_cd_nm}</option>
			                  	</c:forEach>
			                </select>
			                </div>
			                <div class="col-md-auto px-1 py-2">
			                    <input type="text" class="form-control" id="inputCodeName" name="q" value="${param.q}">
			                </div>
	        ...
	        <table class="table table-bordered">
	            <thead>
	                <tr>
	                    <th scope="col" class="table-mob-hidden">그룹코드</th>
	                    <th scope="col">그룹명</th>
	                    <th scope="col" class="table-mob-hidden">공통코드</th>
	                    <th scope="col">공통코드명</th>
	                    <th scope="col">삭제여부</th>
	                    <th scope="col" class="table-mob-hidden">일자</th>
	                </tr>               
	            </thead>
	            <tbody>
		            <c:forEach var="item" items="${list}">
	                <tr>
	                    <th scope="row" class="table-mob-hidden">${item.group_cd}</th>                
	                    <td>${item.group_nm}</th>
	                    <td class="table-mob-hidden"><a href="detail?groupCd=${item.group_cd}&codeCd=${item.code_cd}">${item.code_cd}</a></td>
	                    <td><a href="detail?groupCd=${item.group_cd}&codeCd=${item.code_cd}">${item.code_nm}</a></td>
	                    <td>${item.code_del_nm}</td>
